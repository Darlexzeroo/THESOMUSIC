...
async function saveMessage(senderId, receiverId, text, image, author) {
  if (!isMongoReady()) return null;
  ...
  const saved = await Message.create({ conversationId: conversation._id, senderId, receiverId, text, image });
  return {
    id: String(saved._id),
